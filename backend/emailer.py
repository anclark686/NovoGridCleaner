...
from backend.selenium_funcs import get_bot_pclog
from backend.get_proj_details import get_respondents
from backend.helpers import is_dst

bot, pc_logged = get_bot_pclog()

fvpc = "https://online.focusvision.com/FVPC/default.aspx?Projectid="

with open("backend/json_templates/email_items.json", "r") as file:
...
def send_email(rsp_info, country, lang, pnum, spoc=None, bcc=None):
    ...
    if country == "United States" or country == "Canada":
        session_time = datetime.strptime(rsp_info["rsp_time"], "%H:%M:%S").time()
        sess_str_time = session_time.strftime("%I:%M %p")
...
def grid_emailer():
    with open("backend/json_templates/rsp_combo_details.json", "r") as file:
        data = json.load(file)
        rsps = data["grid_rsps"]
    ...
    for rsp in rsps:
        ...
        if rsp["row_color"] == "yellow" or rsp["row_color"] == "stale yellow":
