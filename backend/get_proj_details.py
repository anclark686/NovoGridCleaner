...
from backend.excel import clean_and_check_grid, make_new_excel
from backend.compare import compare_grid_pc
from backend.helpers import find_country, find_language, define_type
from backend.selenium_funcs import get_bot_pclog, pc_login
...
with open("backend/json_templates/grid_templates.json") as templates_json:
    project_templates = json.load(templates_json)

with open("./frontend/json_templates/projectData.json") as project_data:
...
def save_project_details(info_dict):
    with open("backend/json_templates/project_details.json", "w") as outfile:
        json.dump(info_dict, outfile)


def save_rsp_pc_details(rsp_dict):
    with open("backend/json_templates/rsp_pc_details.json", "w") as outfile:
...
def recheck_dates(master_num):
    ...
    with open("backend/json_templates/project_details.json", "r") as file:
